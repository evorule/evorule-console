...
describe('PluginPacksClient', () => {
    ...
    it('generate: POST JSON body 到 generate 端点', async () => {
        mock.route('POST', /\/templates\/finance-pack\/amount_threshold_approval\/generate$/, () => jsonResponse(GENERATE_BODY));
    ...
    it('HTTP 400 抛 PluginPacksError 且带 status', async () => {
        mock.route('POST', /\/generate$/, () => jsonResponse({ error: '字段 threshold 类型应为 number' }, 400));
