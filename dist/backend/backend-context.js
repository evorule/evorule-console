 ...
 */
export function provideBackend(backend) {
    const instance = backend ?? new HttpBackend();
    setContext(BACKEND_CONTEXT_KEY, instance);
    return instance;
}
/**
...
export function useBackend() {
    if (!hasContext(BACKEND_CONTEXT_KEY)) {
        throw new Error('useBackend: no ExecutionBackend provided. ' +
            'Call provideBackend() in +layout.svelte first.');
    }
    return getContext(BACKEND_CONTEXT_KEY);
}
/**
 * 非抛错版,用于"可能未注入"的场景(如临时组件、e2e 测试页)。
 */
export function useBackendOrNull() {
    if (!hasContext(BACKEND_CONTEXT_KEY))
        return null;
    return getContext(BACKEND_CONTEXT_KEY);
}
