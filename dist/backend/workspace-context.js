 ...
 */
export function provideWorkspaceBackend(backend) {
    const instance = backend ?? new HttpWorkspaceBackend();
    setContext(WORKSPACE_CONTEXT_KEY, instance);
    return instance;
}
/**
...
export function useWorkspaceBackend() {
    if (!hasContext(WORKSPACE_CONTEXT_KEY)) {
        throw new Error('useWorkspaceBackend: no WorkspaceBackend provided. ' +
 ...
 */
export function useWorkspaceBackendOrNull() {
    if (!hasContext(WORKSPACE_CONTEXT_KEY))
        return null;
    return getContext(WORKSPACE_CONTEXT_KEY);
}
