...
| **G2** | 元指令类型(set/push/branch/io_request;公开白名单 4 种 = 全集 − enforce,enforce 仅 tier=meta 文件) | **T1** 元指令枚举 5 种(branch/set/push/io_request/enforce;`collect`/`merge` 已于 v0.6.0 退役,多轮编排由应用层 runner 实现;_shared/v1.0.json transform_rule, 权威源 L0-runtime InstructionExecutor dispatch) | _shared/v1.0.json + TCB_SPEC.md §一 | L0-runtime InstructionExecutor 枚举变体 |
