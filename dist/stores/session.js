...
export const currentWorkspaceSession = writable(null);
 ...
 * @returns 新 session id,失败返回 null
 */
export async function createWorkspaceSession(execBackend, wsBackend, workspaceId, ruleId, ruleVersionId) {
    isLoading.set(true);
    lastError.set(null);
    try {
        ...
        lastError.set(`创建 workspace session 失败: ${e.message}`);
        return null;
    }
    finally {
        isLoading.set(false);
...
/** 当前活跃的 SSE 订阅(防止重复订阅) */
let sessionSwitchedSource = null;
/**
 ...
 */
export function subscribeSessionSwitched(onSwitched) {
    if (!browser)
        return () => { };
    // 防止重复订阅
    ...
    try {
        ...
        source.addEventListener('session_switched', (ev) => {
            try {
