...
export const workspaceSessions = writable([]);
export const workspaceSandboxes = writable([]);
export const publishQueue = writable([]);
export const productionState = writable(null);
 ...
 */
export async function refreshWorkspaces(backend) {
    isLoading.set(true);
    lastError.set(null);
    try {
        ...
        lastError.set(`刷新 workspace 列表失败: ${e.message}`);
    }
    finally {
        isLoading.set(false);
    }
 ...
 */
export async function ensureDefaultWorkspace(backend) {
    // 二次确认(防止并发调用)
    ...
    if (existing.length > 0) {
        ...
        if (browser)
            localStorage.setItem(DEFAULT_WS_KEY, target.id);
        return target;
    }
    ...
    if (browser)
        localStorage.setItem(DEFAULT_WS_KEY, ws.id);
    return ws;
}
/**
 ...
 */
export async function seedBuiltinRules(backend, workspaceId) {
    // 先查重(按 name)
 ...
 */
export async function selectWorkspace(backend, id) {
    isLoading.set(true);
    lastError.set(null);
    try {
        ...
        currentWorkspace.set(ws);
        if (browser)
            localStorage.setItem(DEFAULT_WS_KEY, id);
        ...
        lastError.set(`切换 workspace 失败: ${e.message}`);
    }
    finally {
        isLoading.set(false);
    }
 ...
 * 刷新发布队列。
 */
export async function refreshPublishQueue(backend, status) {
    lastError.set(null);
    try {
        ...
        lastError.set(`刷新发布队列失败: ${e.message}`);
    }
}
/**
 * 刷新生产状态(单行表)。
 */
export async function refreshProductionState(backend) {
    lastError.set(null);
    try {
        ...
        lastError.set(`刷新生产状态失败: ${e.message}`);
 ...
 */
export async function refreshSandboxes(backend, workspaceId) {
    lastError.set(null);
    try {
        ...
        lastError.set(`刷新沙盒列表失败: ${e.message}`);
    }
}
/**
...
export function isRuleReadonly(rule) {
    ...
    if (rule.name.startsWith('example.'))
        return true;
    // metadata 字段判断(二次校验,server 透出后生效)
    if (rule.metadata) {
        try {
            const meta = JSON.parse(rule.metadata);
            if (meta.readonly === true || meta.builtin === true)
                return true;
        }
        catch {
            // metadata 损坏,忽略
