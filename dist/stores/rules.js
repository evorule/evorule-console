...
function toRule(r, content, version) {
    return {
        id: r.id,
...
function persistOfflineSnapshot(workspaceId, list) {
    if (!browser)
        return;
    try {
        localStorage.setItem(OFFLINE_SNAPSHOT_PREFIX + workspaceId, JSON.stringify(list));
    }
    catch {
        // 配额满或隐私模式,忽略
    }
 ...
 */
export async function refreshRules(backend, workspaceId) {
    lastError.set(null);
    try {
        ...
        lastError.set(`刷新规则列表失败: ${e.message}`);
        isOffline.set(true);
        // fallback 到离线快照
...
function loadOfflineSnapshot(workspaceId) {
    if (!browser)
        return;
    try {
 ...
 */
export async function loadRuleContent(backend, workspaceId, ruleId) {
    ...
    try {
        ...
        rules.update((all) => all.map((r) => r.id === ruleId
        ...
        lastError.set(`加载规则内容失败: ${e.message}`);
        return null;
    }
}
/**
 ...
 */
export async function ensureRule(backend, workspaceId, ruleId) {
    ...
    try {
        ...
        lastError.set(`加载规则失败: ${e.message}`);
        return null;
    }
}
/**
 ...
 * @returns 新规则 id (ULID)
 */
export async function addRule(backend, workspaceId, req) {
    lastError.set(null);
    try {
        ...
        lastError.set(`新建规则失败: ${e.message}`);
        throw e;
    }
}
/**
 ...
 */
export async function updateRule(backend, workspaceId, ruleId, patch) {
    const target = get(rules).find((r) => r.id === ruleId);
    if (!target)
        throw new Error(`updateRule: 规则 "${ruleId}" 不存在`);
    // 只读规则禁止修改(替代旧 source==='builtin' 判断)
    if (isRuleReadonly(target)) {
        throw new Error(`updateRule: 只读规则 "${target.name}" 不可修改,请先复制为新副本(duplicateRule)`);
    }
    // 状态校验(仅 Draft 允许编辑内容)
    if (target.state !== 'draft') {
        throw new Error(`updateRule: 规则 "${target.name}" 状态为 ${target.state},仅 draft 状态允许编辑内容`);
    }
    lastError.set(null);
    try {
        ...
        rules.update((all) => all.map((r) => r.id === ruleId ? toRule(record, patch.content, nextVersion) : r));
        persistOfflineSnapshot(workspaceId, get(rules));
    }
    catch (e) {
        lastError.set(`更新规则失败: ${e.message}`);
        throw e;
    }
}
/**
 ...
 * @returns 新规则 id
 */
export async function duplicateRule(backend, workspaceId, sourceId) {
    const all = get(rules);
    const source = all.find((r) => r.id === sourceId);
    if (!source)
        throw new Error(`duplicateRule: 源规则 "${sourceId}" 不存在`);
    if (source.content === undefined) {
        throw new Error(`duplicateRule: 源规则 "${source.name}" 内容未加载,无法复制(阶段 D 补全)`);
    }
 ...
 */
export async function deleteRule(backend, workspaceId, ruleId) {
    const target = get(rules).find((r) => r.id === ruleId);
    if (!target)
        return;
    // 只读规则禁止归档
    ...
    try {
        ...
        lastError.set(`归档规则失败: ${e.message}`);
        throw e;
    }
}
/**
 ...
 * @returns 新规则 id
 */
export async function importRule(backend, workspaceId, jsonContent) {
    let parsed;
    try {
    ...
    const obj = parsed;
    const name = typeof obj.id === 'string' ? `user.${obj.id}` : `user.imported.${Date.now()}`;
    const description = typeof obj.description === 'string' ? obj.description : '(导入的规则,无描述)';
 ...
 */
export async function migrateLegacyRules(backend, workspaceId) {
    if (!browser)
