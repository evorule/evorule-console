...
// Stores
// ============================================================================
export const verdictContracts = writable([]);
export const currentVerdictContract = writable(null);
export const lastEvaluateResult = writable(null);
 ...
 */
export async function refreshVerdictContracts(backend, workspaceId) {
    isLoading.set(true);
    lastError.set(null);
    try {
        ...
        lastError.set(`刷新判定契约失败: ${e.message}`);
 ...
 */
export async function evaluateVerdict(backend, workspaceId, payload, contractId) {
    isLoading.set(true);
    lastError.set(null);
    try {
        ...
        lastError.set(`判定评估失败: ${e.message}`);
        return null;
    }
    finally {
        isLoading.set(false);
 ...
 * 创建判定契约。
 */
export async function createVerdictContract(backend, workspaceId, req) {
    isLoading.set(true);
    lastError.set(null);
    try {
        ...
        lastError.set(`创建判定契约失败: ${e.message}`);
        return null;
    }
    finally {
        isLoading.set(false);
 ...
 * 清空所有状态。
 */
export function resetVerdictStore() {
    verdictContracts.set([]);
    currentVerdictContract.set(null);
