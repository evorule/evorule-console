...
/** 断言叶子节点结构 */
function assertLeaf(node, status, key) {
...
describe('deepDiff — undefined 处理', () => {
    ...
    test('a=undefined + b=对象 → added(容器,保留结构)', () => {
        ...
        r.children.forEach((c) => expect(c.status).toBe('added'));
    });
    test('a=数组 + b=undefined → removed(容器,保留结构)', () => {
        ...
        expect(r.children.length).toBe(2);
        r.children.forEach((c) => expect(c.status).toBe('removed'));
    });
...
describe('deepDiff — 对象', () => {
    ...
    test('嵌套对象递归 diff', () => {
        const r = deepDiff({ outer: { a: 1, b: 2 } }, { outer: { a: 1, b: 99 } });
        const outer = findChild(r, 'outer');
        expect(outer.kind).toBe('object');
    ...
    test('子键按字母顺序排序', () => {
        const r = deepDiff({ z: 1, a: 2, m: 3 }, { z: 1, a: 2, m: 3 });
...
describe('deepDiff — 数组', () => {
    ...
    test('嵌套数组递归 diff', () => {
        const r = deepDiff([[1, 2], [3, 4]], [[1, 2], [3, 99]]);
    ...
    test('数组元素为对象 → 递归 diff', () => {
        const r = deepDiff([{ x: 1 }, { y: 2 }], [{ x: 1 }, { y: 99 }]);
...
describe('countChanges', () => {
    ...
    test('对象多字段变化 → N', () => {
        const r = deepDiff({ a: 1, b: 2, c: 3 }, { a: 11, b: 22, c: 33 });
        expect(countChanges(r)).toBe(3);
    });
    test('对象增删改混合 → 分别计数', () => {
        const r = deepDiff({ a: 1, b: 2, c: 3 }, { a: 1, b: 99, d: 4 });
    ...
    test('深嵌套变更计数', () => {
        const r = deepDiff({ outer: { a: 1, b: 2, c: { x: 10, y: 20 } } }, { outer: { a: 99, b: 2, c: { x: 10, y: 99 } } });
