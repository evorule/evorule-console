<!--
  ...
  evorule-console 流程设计页（通用画布外壳,Plugin Contract v1.1 §5/§6,第二期 Phase C）
-->
<!--
  职责:
  ...
  数据流:PluginPacksClient（默认 127.0.0.1:18080;--auth-token 时经 localStorage 传 Bearer）
-->
...
<script lang="ts">
  ...
  import FlowCanvas from "$lib/views/FlowCanvas/FlowCanvas.svelte";
  import {
    defaultPosition,
    buildFlowJson,
    loadFlowAsset,
...
<div class="flow-page">
  ...
  <header class="flow-topbar">
    <div class="brand">
      ...
      <span class="brand-section">流程设计</span>
    </div>
