...
function entry(fact_id: number, cause: number | null = null, extra: Record<string, any> = {}): any {
  return {
    fact_id,
...
/** 按 fact_id 查找布局后的节点 */
function findNode(result: any, fact_id: number): any {
  return result.nodes.find((n: any) => n.fact_id === fact_id);
}
...
describe('layoutDag — 线性链(A→B→C)', () => {
  ...
  test('X 定位:每层 1 节点,x=0', () => {
    r.nodes.forEach((n: any) => expect(n.x).toBe(0));
  });
  ...
  test('边正确:{1→2, 2→3}', () => {
    expect(r.edges.some((e: any) => e.from === 1 && e.to === 2)).toBe(true);
    expect(r.edges.some((e: any) => e.from === 2 && e.to === 3)).toBe(true);
  });
});
...
describe('layoutDag — 深层分支(A→B→D, A→C→E)', () => {
  ...
  test('4 条边', () => {
    ...
    expect(r.edges.some((e: any) => e.from === 1 && e.to === 2)).toBe(true);
    expect(r.edges.some((e: any) => e.from === 1 && e.to === 3)).toBe(true);
    expect(r.edges.some((e: any) => e.from === 2 && e.to === 4)).toBe(true);
    expect(r.edges.some((e: any) => e.from === 3 && e.to === 5)).toBe(true);
  });
...
describe('layoutDag — 森林(多 root)', () => {
  ...
  test('3 个独立 root → 同 layer 0,三节点均匀分布', () => {
    ...
    nodes.forEach((n: any) => expect(n.layer).toBe(0));

    // 间距一致
    const sorted = [...nodes].sort((a: any, b: any) => a.x - b.x);
...
describe('layoutDag — 尺寸计算', () => {
  ...
  test('空输入尺寸为 0', () => {
    expect(layoutDag(null as any).width).toBe(0);
    expect(layoutDag(null as any).height).toBe(0);
...
describe('layoutDag — 综合:evorule causal chain', () => {
  test('典型 causal chain 布局', () => {
    ...
    // Y 递增
    const ys = r.nodes.map((n: any) => n.y);
  ...
  test('带分支的复杂因果图', () => {
    ...
    chain.forEach((e: any) => { if (e.cause != null) parentOf.set(e.fact_id, e.cause); });
    r.nodes.forEach((n: any) => {
      if (n.cause != null) {
