...
export interface BusinessFieldDef {
  id: BusinessFieldId;
  label: string; // "金额"
  path: string; // "__exec__.payload.amount"
  inputType: 'number' | 'text';
}
export const BUILTIN_FIELDS: BusinessFieldDef[] = [
...
const DOMAIN_FIELD_REGISTRY: Record<string, BusinessFieldDef[]> = {};
export function registerDomainFields(domain: string, fields: BusinessFieldDef[]): void {
  DOMAIN_FIELD_REGISTRY[domain] = fields;
}
/** 取所有可用字段(内置 + 已注册领域) */
export function allFields(): BusinessFieldDef[] {
  const domainFields = Object.values(DOMAIN_FIELD_REGISTRY).flat();
  return [...BUILTIN_FIELDS, ...domainFields];
}
/** 按 id 查字段定义 */
export function findField(id: BusinessFieldId): BusinessFieldDef | undefined {
  return allFields().find((f) => f.id === id);
}
/** 按 path 反查字段定义(回译用) */
export function findFieldByPath(path: string): BusinessFieldDef | undefined {
  return allFields().find((f) => f.path === path);
}

// === 业务比较符(决策1后,console 唯一面对的 op) ===
export type BusinessOp = 'gte' | 'gt' | 'eq' | 'lt';
export const BUSINESS_OPS: { id: BusinessOp; symbol: string; label: string }[] = [
  { id: 'gte', symbol: '≥', label: '大于等于' },
  { id: 'gt', symbol: '>', label: '大于' },
  { id: 'eq', symbol: '=', label: '等于' },
  { id: 'lt', symbol: '<', label: '小于' },
];
export function opSymbol(op: BusinessOp): string {
  return BUSINESS_OPS.find((o) => o.id === op)?.symbol ?? op;
}

// === 业务动作(决策2后,去掉 block) ===
export type BusinessAction = 'notify' | 'approve' | 'flag';
export const BUSINESS_ACTIONS: { id: BusinessAction; label: string; desc: string }[] = [
  { id: 'notify', label: '通知', desc: '打"需通知"标记' },
  { id: 'approve', label: '审批', desc: '打"需审批"标记' },
  { id: 'flag', label: '关注', desc: '打"需关注"标记' },
];
export function actionLabel(a: BusinessAction): string {
  return BUSINESS_ACTIONS.find((x) => x.id === a)?.label ?? a;
}
/** 动作对应的 result 路径 */
export function actionPath(a: BusinessAction): string {
...
// === 自然语言摘要 ===
export function summarize(rule: Pick<BusinessRule, 'conditions' | 'actions'>): string {
 ...
 */
export function businessRuleToTranslateInput(
  rule: Pick<BusinessRule, 'conditions' | 'actions'>
): { condition: unknown[]; action_set: unknown[] } {
 ...
 */
export function translateOutputToBusinessRule(
  condition: unknown[],
  action_set: unknown[],
  lossy: boolean,
  lostItems: string[]
): Pick<BusinessRule, 'conditions' | 'actions' | 'unsupported' | 'unsupportedReason'> {
  const conditions: BusinessCondition[] = [];
  ...
  for (const c of condition) {
    ...
    const op = String(obj.op ?? '') as BusinessOp;
  ...
  for (const a of action_set) {
    ...
    if (match && value && typeof value.role === 'string') {
      actions.push({ type: match[1] as BusinessAction, role: value.role });
    } else {
